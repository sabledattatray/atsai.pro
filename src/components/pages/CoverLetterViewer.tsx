...
import { generateCoverLetter, CoverLetterStyle, getBaseRole, CoverLetterStructuredContent } from '@/lib/coverLetterGenerator';
      ...
      <div className={`sticky top-0 z-30 print:hidden transition-colors duration-300 border-b ${theme === 'dark' ? 'bg-slate-900 border-slate-800' : 'bg-white border-gray-200'}`}>
        <div className="container mx-auto px-4 max-w-7xl py-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            <Link href="/" className={`w-10 h-10 rounded-full flex items-center justify-center transition-colors ${theme === 'dark' ? 'bg-slate-800 hover:bg-slate-700 text-slate-350' : 'bg-gray-100 hover:bg-gray-200 text-gray-600'}`}>
