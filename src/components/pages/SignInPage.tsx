...
import { auth, googleProvider, githubProvider, signInWithRedirect, getRedirectResult } from '@/lib/firebase';
...
export default function SignInPage() {
  ...
  // Handle redirect result after Google/GitHub OAuth
  useEffect(() => {
    if (!auth) return;
    ...
    if (auth) {
      try {
        ...
        if (err.code === 'auth/operation-not-allowed') {
          setErrorMsg('Email/Password provider is not enabled in your Firebase Console.');
  ...
  if (oauthLoading) {
    return (
      <div className="flex w-full min-h-screen bg-[#030712] items-center justify-center">
        ...
        <div className="relative z-10 max-w-md">
          ...
          <div className="mt-8 space-y-4">
            ...
            <div className="flex items-start gap-3 text-sm text-slate-300">
              <div className="flex-shrink-0 w-5 h-5 rounded-full bg-purple-500/10 flex items-center justify-center border border-purple-500/20 mt-0.5">
                <svg className="w-3 h-3 text-purple-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="3"><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" /></svg>
              </div>
              <div>
          ...
          <div className="mt-12 pt-6 border-t border-white/5">
            ...
            <div className="flex flex-wrap gap-x-6 gap-y-2 opacity-50 select-none items-center">
              <span className="text-xs font-black tracking-widest text-slate-300">GOOGLE</span>
              <span className="text-xs font-black tracking-widest text-slate-300">META</span>
              <span className="text-xs font-black tracking-widest text-slate-300">NETFLIX</span>
              <span className="text-xs font-black tracking-widest text-slate-300">STRIPE</span>
              <span className="text-xs font-black tracking-widest text-slate-300">MICROSOFT</span>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="w-full max-w-md relative z-10">
          ...
          <div className="grid grid-cols-2 gap-3">
            <button
              ...
              className="h-12 flex items-center justify-center gap-2 rounded-xl border border-white/10 bg-white/5 hover:bg-white/10 text-white font-bold text-[10px] uppercase tracking-widest transition-all cursor-pointer select-none"
            >
